...
//Repara totalmente o crossover
void adjustCrossover(int a[], int vert){
    int count=0;
    int checkVal[1000];
    
     for(int k=0; k< vert; k++) //guarda na posiçao correspondente ao valor presente na soluçao a qty das suas ocorrencias
            checkVal[a[k]-1] += 1;
        ...
        while(count != vert)
        {
            count =0;
             ...
             {
                if(checkVal[k] == 1) count++; //se for ==1(como desejado, sem repeticoes) 
                                              //aumenta um contador que serve para ver quando ja nao ha mais invalidos
                if(checkVal[k] > 1) //se for maior que 1 repete se 
                    for(int j=0; j < vert; j++) //no soluçao procuramos o valor 
                                                //correspondente ao valor de k+1, aka a 1a vez que o valor repetido se repete
                      if(a[j] == k+1){
                          for(int x=0; x<vert; x++) //para um repetir tem de haver um outro que nao tem ocorrencia
                              if(checkVal[x]==0) //procuramos o que nao tem ocorrencia
                              {
                                  a[j]=x+1; //e colocamo lo no lugar do repetido na soluçao
                                   for(int k=0; k< vert; k++)checkVal[k] = 0;
                                  for(int k=0; k< vert; k++) checkVal[a[k]-1] += 1; //atualiza o checkRep array
                                  break;
                              }break;}   
             }
        }//aplicamos este algoritmo ate nao haver mais reps
       for(int k=0; k< vert; k++)
           checkVal[k] = 0;
}
